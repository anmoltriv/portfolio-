    ...
    >
      ...
      <div className="space-y-6">
        {experiencesData.map((exp) => (
          <TiltCard
            key={exp.id}
            className={`bg-white/[0.0125] border border-white/10 rounded-2xl p-6 md:p-8 overflow-hidden hover:bg-white/[0.025] ${tokens.cardHover}`}
            maxTilt={4}
          >
            <div className={`absolute top-6 right-6 text-[10px] font-mono tracking-widest uppercase font-semibold ${tokens.text}`}>
              {exp.period}
            </div>

            <div className="flex items-start gap-4 mb-4 relative z-10">
              <div className={`w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 ${tokens.badgeBg} ${tokens.text} ${tokens.border}`}>
            ...
            <ul className="space-y-3 mt-4 ml-2 pl-4 border-l border-white/10 text-sm text-white/70 leading-relaxed list-none relative z-10">
